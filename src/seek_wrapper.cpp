...
SeekWrapper::SeekWrapper(const std::shared_ptr<rclcpp::Node> nh)
    : nh_(nh)
{
    // Param setup
    nh_->get_parameter_or("do_calibrate", calibration_mode_, calibration_mode_);
    nh_->get_parameter_or("offline", offline_, offline_);
    ...
    // ROS setup
    image_pub_ = nh_->create_publisher<sensor_msgs::msg::Image>("image_raw", 10);
    thermal_pub_ = nh_->create_publisher<sensor_msgs::msg::Image>("image_thermal", 10);
    info_pub_ = nh_->create_publisher<sensor_msgs::msg::CameraInfo>("camera_info", 10);
    // set_info_service_ = nh_->create_service("set_camera_info", &setCameraInfo);
    // if (offline_) {
    //     image_sub_ = nh_->create_subscription<sensor_msgs::msg::Image>("image_raw", 10, std::bind(&SeekWrapper::imageCallback, nh_, _1));
    // }

    if (!offline_) {
        // Create the camera manager.
        // This is the structure that owns all Seek camera devices.
        seekcamera_error_t status = seekcamera_manager_create(&manager_, SEEKCAMERA_IO_TYPE_USB);
        ...
        // status = seekcamera_manager_register_event_callback(manager_, seekcamera_manager_event_callback_t (&SeekWrapper::camera_event_callback), nullptr);
        // if (status != SEEKCAMERA_SUCCESS)
        // {
...
// TODO bring back camera calibration methods in Humble
// bool SeekWrapper::setCameraInfo(sensor_msgs::srv::SetCameraInfo::Request& req, sensor_msgs::srv::SetCameraInfo::Response& resp) {
...
void SeekWrapper::imageCallback(const sensor_msgs::msg::Image::SharedPtr &image) {
    camera_info_.header = image->header;
    // info_pub_->publish(*camera_info_);
}

// Handles frame available events.
void SeekWrapper::handle_camera_frame_available(seekcamera_t *camera, seekcamera_frame_t *camera_frame, void *user_data)
{
    ...
    // info_pub_->publish(*camera_info_.get());
}

// Handles camera connect events.
void SeekWrapper::handle_camera_connect(seekcamera_t *camera, seekcamera_error_t event_status, void *user_data)
{
    (void)event_status;
    (void)user_data;
    ...
    seekcamera_error_t status;// = seekcamera_register_frame_available_callback(camera, &SeekWrapper::handle_camera_frame_available, nullptr);
    // if (status != SEEKCAMERA_SUCCESS)
    // {
...
void SeekWrapper::handle_camera_disconnect(seekcamera_t *camera, seekcamera_error_t event_status, void *user_data)
{
    (void)event_status;
    (void)user_data;
}
...
void SeekWrapper::handle_camera_error(seekcamera_t *camera, seekcamera_error_t event_status, void *user_data)
{
    (void)user_data;
    seekcamera_chipid_t cid{};
    seekcamera_get_chipid(camera, &cid);
...
void SeekWrapper::handle_camera_ready_to_pair(seekcamera_t *camera, seekcamera_error_t event_status, void *user_data)
{
...
void SeekWrapper::camera_event_callback(seekcamera_t *camera, seekcamera_manager_event_t event, seekcamera_error_t event_status, void *user_data)
{
    seekcamera_chipid_t cid{};
    seekcamera_get_chipid(camera, &cid);
